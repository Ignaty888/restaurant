...
{
    public partial class TypeDishesForm : Form
    {
        /// <summary>
        ...
        /// Конструктор формы
        /// </summary>
        public TypeDishesForm(Repository repository)
        {
        ...
        private void UpdateDatagrid()
        {
            ...
            dataGridView.DataSource = _repository.GetEntityes<TypeDish>();
        }
        ...
        private void dataGridView_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == dataGridView.Columns["EditColumn"].Index)
            {
                ...
                var item = dataGridView.SelectedRows[0].DataBoundItem as TypeDish;
                // передаём полученный объект в конструктор формы редактирования и показываем форму
                new AddEditTypeDishForm(item,_repository).ShowDialog();
            ...
            if (e.ColumnIndex == dataGridView.Columns["DeleteColumn"].Index)
            {
                ...
                var result = MessageBox.Show("Удалить объект с кодом " + item.ID_TypeDish,"",MessageBoxButtons.OKCancel,MessageBoxIcon.Question);
                // если ответ не положительный - выйти из метода
                if (result!=DialogResult.OK) return;
                // в противном случае попытаться удалить объект с БД
                try
                {
                    _repository.RemoveItem<TypeDish>(item); // удаляем объект
        ...
        private void buttonAdd_Click(object sender, EventArgs e)
        {
            ...
            new AddEditTypeDishForm(_repository).ShowDialog();
