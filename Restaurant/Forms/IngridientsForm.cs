...
namespace Restaurant.Forms
{
    public partial class IngridientsForm : Form
    {
        ...
        /// Конструктор формы
        /// </summary>
        public IngridientsForm(Repository repository)
        {
        ...
        private void UpdateDatagrid()
        {
            ...
            dataGridView.DataSource = _repository.GetEntityes<Ingredient>();
        }
        ...
        private void dataGridView_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == dataGridView.Columns["EditColumn"].Index)
            {
                ...
                var item = dataGridView.SelectedRows[0].DataBoundItem as Ingredient;
                // передаём полученный объект в конструктор формы редактирования и показываем форму
                new AddEditIngridientForm(item, _repository).ShowDialog();
            ...
            if (e.ColumnIndex == dataGridView.Columns["DeleteColumn"].Index)
            {
                ...
                var result = MessageBox.Show("Удалить объект с кодом " + item.ID_Ingredient,"",MessageBoxButtons.OKCancel,MessageBoxIcon.Question);
                // если ответ не положительный - выйти из метода
                if (result!=DialogResult.OK) return;
                // в противном случае попытаться удалить объект с БД
                try
                {
                    _repository.RemoveItem<Ingredient>(item); // удаляем объект
        ...
        private void buttonAdd_Click(object sender, EventArgs e)
        {
            ...
            new AddEditIngridientForm(_repository).ShowDialog();
